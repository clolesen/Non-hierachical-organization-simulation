  ...
  for (round in 1:nR){
    ...
    for (p in 1:nP){
      ...
      if (work[p] <= maxOpt){ 
        ...
        #rewards[p] = maxOpt + (work[p]-maxOpt+1)^rewardExp-1 # The exponential function
        rewards[p] = maxOpt + (work[p]-maxOpt+1)*(2/12)
      }
...
write.csv2(data3pAll, "data/data_3projects.csv")
...
write.csv2(data6pAll, "data/data_6projects.csv")
...
simPlots = function(data,projects,sym=T){
  ...
  if (projects == 3) {
    ...
    plot8 = ggplot(data, aes(y = Reward, x = Agent)) +
      ...
      facet_wrap(~Simulation) +
      theme_minimal() +
      labs(title = "Bar plot of the total sum of rewards per agent", subtitle = "Across 9 different simulations of 30 rounds each", y = "Sum of rewards")
    ...
    plot9 = ggplot(subset(data, Agent %in% c(1,9)), aes(x = Round, y = as.factor(Choice))) +
      ...
      facet_wrap(~Simulation) +
      theme_minimal() +
      labs(title = "Agent 1 and 9 project choice", subtitle = "Across 9 different simulations of 30 rounds each", y = "Projects", color = "Agent")
    ...
    plot11 = ggplot(data, aes(x = Round))+
      geom_line(aes(y = Project3_Tasks+Project2_Tasks+Project1_Tasks)) +
      facet_wrap(~Simulation) +
      theme_minimal() +
      labs(title = "Tasks left each round",subtitle = "All projects", y = "Tasks")
...
plot10 = ggplot(data, aes(x = Round, y = as.factor(Agent))) +
  ...
  if (projects == 6){
    ...
    plot14 = ggplot(data, aes(x = Round))+
      geom_line(aes(y = Project1_Tasks+Project2_Tasks+Project3_Tasks+Project4_Tasks+Project5_Tasks+Project6_Tasks)) +
      facet_wrap(~Simulation) +
      theme_minimal() +
      labs(title = "Tasks left each round",subtitle = "All projects", y = "Tasks")
...
# Loops that saves plots in the right folders
for (a in c(18,40,60,90,120)){
  for (p in c(3,6)){
    for (sym in c("asymmetric", "symmetric")){
      path = paste0("plots/30_rounds/",a,"_agents/",p,"_projects/",sym,"/")
      
      if (p == 3){
        plots = simPlots(subset(data3pAll, nAgents == a & symmetry == sym),3)
        
        ggsave(paste0(path,"Tasks-left_plot.png"), plots[[1]])
        ggsave(paste0(path,"Tasks-done_plot.png"), plots[[2]])
        ggsave(paste0(path,"Reward-given_agents_plot.png"), plots[[3]])
        ggsave(paste0(path,"Rewards-given_summary_plot.png"), plots[[4]])
        ggsave(paste0(path,"Agent1and9_project1_plot.png"), plots[[5]])
        ggsave(paste0(path,"Agent1and9_project2_plot.png"), plots[[6]])
        ggsave(paste0(path,"Agent1and9_project3_plot.png"), plots[[7]])
        ggsave(paste0(path,"Sum-rewards_plot.png"), plots[[8]])
        ggsave(paste0(path,"Agent1and9_choice_plot.png"), plots[[9]])
        ggsave(paste0(path,"Project_choice_plot.png"), plots[[10]])
        ggsave(paste0(path,"Tasks-left-total_plot.png"), plots[[11]])
      }
      
      if (p == 6){
        plots = simPlots(subset(data6pAll, nAgents == a & symmetry == sym),6)
        ...
        ggsave(paste0(path,"Agent1and9_project2_plot.png"), plots[[6]])
        ggsave(paste0(path,"Agent1and9_project3_plot.png"), plots[[7]])
        ggsave(paste0(path,"Agent1and9_project4_plot.png"), plots[[8]])
        ggsave(paste0(path,"Agent1and9_project5_plot.png"), plots[[9]])
        ggsave(paste0(path,"Agent1and9_project6_plot.png"), plots[[10]])
        ggsave(paste0(path,"Sum-rewards_plot.png"), plots[[11]])
        ggsave(paste0(path,"Agent1and9_choice_plot.png"), plots[[12]])
        ggsave(paste0(path,"Project_choice_plot.png"), plots[[13]])
        ggsave(paste0(path,"Tasks-left-total_plot.png"), plots[[14]])
      }
    }
  }
}
...
```{r}
write.csv2(subset(data3pAll, Round == 30 & Agent == 1)[,c(1,9:11,18,21)], "data/tasks_left_round30_3projects.csv")

write.csv2(subset(data6pAll, Round == 30 & Agent == 1)[,c(1,12:17,30,33)], "data/tasks_left_round30_6projects.csv")
```
